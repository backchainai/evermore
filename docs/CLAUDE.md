...
**Not a git repository.** The top-level `evermore/` directory has no `.git`, so changes under `docs/` have no version-control safety net. Treat moves, renames, and deletes as permanent, and get explicit user approval before deleting anything.
...
| `research/source-research/` | Tier 1 (primary) | citable evidence base (RAG-indexable) | Peer-reviewed studies, grouped by domain |
| `research/reference-library/` | Tier 1 (operational) | Retriever shelter-ops assistant | Handbooks, training, body-language, DPFL library |
...
- **Log structural changes** (moves, renames) to `research/_reorg-manifest.md` so the un-versioned tree stays reversible.
