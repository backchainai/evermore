...
In Evermore, **"extraction" means retrieving existing data out of a shelter system** (a Pet Data module capability, not yet built). That sense of extraction is distinct from the **distilled** Tier-2 research rules in `research/distilled/`. Do not conflate the two.
