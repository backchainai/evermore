...
- The design partner's name was scrubbed from the working tree in Phase 3: code, tests, and docs are genericized to "Shelter Management System (SMS)". Do not reintroduce client names or branded SMS tool names; refer to shelter systems generically. Exception, decided by the project owner on 2026-09-09: a module that integrates with a specific shelter system names that system, in its directory and in its documentation. In the owner's words, "THey are not generic in modules that interact with them, they should be specifically named in those cases." The core, and anything not specific to one vendor, stays generic. The exception covers vendor names only and does not relax the client-name ban above: the design partner is never named. The pre-Phase-3 snapshot-import commit still contains the design partner's name in git history; a history scrub is a separate prerequisite before the repo flips public.
