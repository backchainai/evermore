"""Database migration engine for petbio.
...
Example:
    Initialize a new database:
        ...
        >>> init_database(Path("data/petbio.db"))

    Apply pending migrations:
        >>> from petbio.modules.db import migrate
        >>> migrate(Path("data/petbio.db"))
...
def _scan_migration_files(migrations_dir: Path) -> Sequence[Migration]:
    ...
    Example:
        >>> migrations = _scan_migration_files(Path("src/petbio/modules/db/migrations"))
...
def apply_migration(db_path: Path, migration: Migration) -> None:
    ...
    Example:
        ...
        ... )
        >>> apply_migration(Path("data/petbio.db"), migration)
    """
...
def migrate(db_path: Path, target_version: int | None = None) -> None:
    ...
    Example:
        ...
        >>> migrate(Path("data/petbio.db"), target_version=3)
    """
