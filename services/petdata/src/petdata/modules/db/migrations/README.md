...
This directory contains SQL migration files for evolving the petdata database schema.
...
If the migration adds/changes tables, update corresponding Pydantic models in `src/petdata/modules/db/models.py` to match the new schema.
...
migrate(Path("data/petdata.db"))  # Applies all pending migrations
```

### Check Pending Migrations
```python
from petdata.modules.db.migrate import get_pending_migrations
from pathlib import Path
